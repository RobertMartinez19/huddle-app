  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <div style={{ fontFamily: "Oswald", fontWeight: 600, fontSize: 18 }}>Group Pools</div>
            ...
            <Card key={pool.id}>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <div>
                  ...
                  <div style={{ fontFamily: "Oswald", fontWeight: 600, fontSize: 16, marginTop: 6 }}>{pool.title}</div>
                  <div style={{ fontSize: 11, color: "var(--text-dim)" }}>{game.a} vs {game.b} · entry {fmt(pool.entryFee)} pts</div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontSize: 10, color: "var(--text-dim)" }}>POT</div>
                  <PointsTicker value={potTotal} size={18} />
                  <div style={{ fontSize: 10, color: "var(--text-dim)" }}>rake {fmt(rake)}</div>
                </div>
              </div>
              ...
              <div style={{ display: "flex", gap: 6, marginTop: 12, flexWrap: "wrap" }}>
                {pool.picks.map((pk) => (
                  <div key={pk.friend} style={{
                    ...
                    borderRadius: 999, padding: "3px 8px 3px 3px",
                    border: pool.status === "settled" && pk.side === pool.resultSide ? "1px solid var(--green)" : "1px solid transparent",
                  }}>
                    <Avatar name={pk.friend} size={18} />
                    <span style={{ fontSize: 11 }}>{pk.side}</span>
                  </div>
                ))}
              </div>
              ...
              {pool.status === "open" && myPick && (
                <div style={{ marginTop: 10, fontSize: 12, color: "var(--green)", display: "flex", alignItems: "center", gap: 5 }}>
